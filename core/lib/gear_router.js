...
module.exports=class GearRouter{
    ...
    route(handle){
        ...
        handle.res.writeHeader(200,{
            'Content-Type':'text/html;charset='+this.gear.options.charset,
        });
        !this.gear.options.debug || handle.dump(handle.host+handle.req.url); //debug模式下，控制台打印请求地址

        for (let i in rule){
            let item=rule[i];
            let rel=false;
            if(item.path===handle.path ||
                item.preg&&item.preg.test(handle.path)){
